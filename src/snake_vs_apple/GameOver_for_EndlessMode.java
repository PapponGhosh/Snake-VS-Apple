...
public class GameOver_for_EndlessMode implements ActionListener{
	
	JFrame f;
	JButton quit_button;
	JButton menu_back_button;

	GameOver_for_EndlessMode(int score) throws IOException{
		
		//High Score
		BufferedReader r = new BufferedReader(new FileReader("endless score.txt"));
		...
		JLabel show_high_score = new JLabel("High Score!");				//it will be used if the it's really the high score
		...
		if(high_s)
		p.add(show_high_score);		//it will show "high score!" only if it's really the high score
