...
public class Level_2 extends JPanel implements ActionListener{
	...
	Level_2(int score, int snake_running_time){
		this.score = score;
	...
	public void draw(Graphics g) {
		if(running) {
			...
			//apple
			g.drawImage(fruit.getImage(), ax, ay, os, os, null);
			
			//pipes
			for(int i=100; i<=600; i++) {
				if(i==60) {
					g.drawImage(pipe_up.getImage(), 197, i, os+6, os, null);
					g.drawImage(pipe_up.getImage(), 797, i, os+6, os, null);
				}
				else if(i==640) {
					g.drawImage(pipe_down.getImage(), 196, i, os+6, os, null);
					g.drawImage(pipe_down.getImage(), 796, i, os+6, os, null);
				}
				else {
					g.drawImage(pipe_middle.getImage(), 200, i, null);
					g.drawImage(pipe_middle.getImage(), 800, i, null);
				}
			}
	...
	public void newApple() {
		...
		while(true) {
			if(((ay>=60 && ay<=640) && (ax==200 || ax==800)) || (ay<=20)) {
				ax = random.nextInt((int)(sw/os))*os;
				ay = random.nextInt((int)(sh/os))*os;
			}
			else
	...
	public void checkCol() {
		...
		for(int i=100; i<=600; i++) {
			if((x[0]==200 && y[0]==i) || (x[0]==800 && y[0]==i)) {
				running = false;
			}
		}
